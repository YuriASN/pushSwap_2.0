...
R=$(valgrind --leak-check=full --show-leak-kinds=all --error-exitcode=111 2> /dev/null ./push_swap $ARG > /dev/null && echo $?)
if [[ $R == 0 ]]; then
...
S=$(echo -e "pb\npb\nss\npa\npa" | ./checker_linux $ARG)
R=$(echo -e "pb\npb\nss\npa\npa" | ./checker $ARG)
L=$(echo -e "pb\npb\nss\npa\npa" | valgrind --leak-check=full --show-leak-kinds=all --error-exitcode=111 2> /dev/null ./checker $ARG > /dev/null && echo $?)
if [ $S == $R ]; then
  printf "${GREEN}3.[OK] ${DEF_COLOR}";
else
  printf "${RED}3.[KO]${DEF_COLOR}";
...
R=$(echo -e "sa\nsb\nss\npa\npb\npa\npb\nsb\npa\nra\nrb\nrr\nrra\nrrb\nrrr" | ./checker $ARG)
L=$(echo -e "sa\nsb\nss\npa\npb\npa\npb\nsb\npa\nra\nrb\nrr\nrra\nrrb\nrrr" | valgrind --leak-check=full --show-leak-kinds=all --error-exitcode=111 2> /dev/null ./checker $ARG > /dev/null && echo $?)
if [ $S == $R ]; then
  printf "${GREEN}6.[OK] ${DEF_COLOR}";
else
  printf "${RED}6.[KO] ${DEF_COLOR}";
...
R=$(./push_swap $ARG | valgrind --leak-check=full --show-leak-kinds=all --error-exitcode=111 2> /dev/null ./checker $ARG > /dev/null && echo $?)
if [[ $R == 0 ]]; then
  printf "${GREEN}[MOK] ${DEF_COLOR}";
else
  printf "${RED} [KO LEAKS] ${DEF_COLOR}";
